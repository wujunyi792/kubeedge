...
func NewEdgeJoin() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PreRunE: func(cmd *cobra.Command, args []string) error {
			...
			running, _ := util.IsKubeEdgeProcessRunning(util.KubeEdgeServiceName)
			if running {
...
func createEdgeConfigFiles(opt *common.JoinOptions) error {
	// Determines whether the kubeEdgeVersion is earlier than v1.14.0
	// windows edge core only support v1.14+
	...
	if v.Major <= 1 && v.Minor < 14 {
		return fmt.Errorf("KubeEdge Edgecore on Windows only support v1.14+")
	}
	if opt.RuntimeType != constants.DefaultRuntimeType {
		return fmt.Errorf("since KubeEdge v1.14, runtime type only supports `remote`")
	}

	configFilePath := util.KubeEdgeEdgeCoreNewYaml
...
func runEdgeCore() error {
	_err := common.InstallWindowsService(util.KubeEdgeUsrBinPath+"\\"+util.KubeEdgeBinaryName, util.KubeEdgeServiceName,
		"edgecore", "Kubeedge Edgecore on windows", "--config "+util.KubeEdgeEdgeCoreNewYaml)
	if _err != nil {
		return _err
	}
